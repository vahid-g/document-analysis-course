...
public class ArffData {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (data_started) {
					DataEntry d = null;
					if (line.indexOf('{') >= 0)
						d = addSparseDataEntry(StripBraces(line));
					else
						d = addDataEntry(line);
					//System.out.println("Added data entry: " + d);
					continue;
				}
	...
	public class Attribute {
		...
		public int addClassVal(String val) {
			if (class_id_map.containsKey(val))
				return (Integer)class_id_map.get(val);
			val = StripQuotes(val);
	...
	public static void main(String args[]) {
		...
		ArffData f1 = new ArffData("data/arff/vote.arff");
		//ArffData f1 = new ArffData("data/arff/vote_sparse.arff");
		System.out.println("Data: " + f1);

		// Build a single split of the data and write to disk
		...
		// Build a 10-fold split of the data and write to disk
